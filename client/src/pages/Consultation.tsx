...
import { MessageSquare, Phone, Video, Star, Clock } from "lucide-react";
import { toast } from "sonner";
import { MOCK_DOCTORS, createConsultation, type Doctor } from "@/lib/mockData";
...
const Consultation = () => {
  ...
  const handleBookConsultation = (doctor: Doctor, mode: ConsultMode) => {
        ...
        <div className="flex items-center justify-between mb-6">
          ...
          {doctors.length > 0 && (
            <p className="text-sm text-muted-foreground">
              {doctors.filter(d => d.availability === "online").length} doctors available for instant consultation
            </p>
          )}
        </div>
